...
proc nVec3*(x, y, z: float): Vec3 = Vec3(x: x, y: y, z: z)

# scalar operators
proc `*`*(v: Vec3, a: float): Vec3 = nVec3(v.x*a, v.y*a, v.z*a)
proc `/`*(v: Vec3, a: float): Vec3 = nVec3(v.x/a, v.y/a, v.z/a)
...
proc `*`*(v1: Vec3, v2: Vec3): float = v1.x*v2.x + v1.y*v2.y + v1.z*v2.z

proc negate*(v: Vec3): Vec3 = nVec3(-v.x, -v.y, -v.z)

proc length*(v: Vec3): float = math.sqrt(v.x*v.x + v.y*v.y + v.z*v.z)

proc setLengthTo*(v: Vec3, a: float): Vec3 = v * (a / v.length)
